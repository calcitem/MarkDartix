...
if (!/^(darwin|win32|linux)$/i.test(process.platform)) {
  process.stdout.write(`Operating system "${process.platform}" is not supported! Please open an issue at "https://github.com/calcitem/markdartix".\n`)
  process.exit(1)
}
...
// Make MarkDartix a single instance application.
if (!process.mas && process.env.NODE_ENV !== 'development') {
  const gotSingleInstanceLock = app.requestSingleInstanceLock()
  if (!gotSingleInstanceLock) {
    process.stdout.write('Other MarkDartix instance detected: exiting...\n')
    app.exit()
  }
}

// MarkDartix environment is configured successfully. You can now access paths, use the logger etc.
...
try {
  ...
  const msgHint = err.message.includes('Config schema violation')
    ...
    : ''
  log.error(`Loading MarkDartix failed during initialization! ${msgHint}`, err)

  const EXIT_ON_ERROR = !!process.env.MARKDARTIX_EXIT_ON_ERROR
  const SHOW_ERROR_DIALOG = !process.env.MARKDARTIX_ERROR_INTERACTION
...
const markdartix = new App(accessor, args)
markdartix.init()
